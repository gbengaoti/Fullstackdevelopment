...
try:
	h = Http()
	h.add_credentials('12345', 'blank')
	url = address + '/products'
	resp, content = h.request(url, 'GET', headers = {"Content-Type" : "application/json"})
	if resp['status'] != '200':
		raise Exception('Received an unsuccessful status code of %s' % resp['status'])
except Exception as err:
	print "Test 4 PASSED: Could not add new products with INVALID token"	
	print err.args
else:
	print "Test 4 FAILED: Succesfully added new products with INVALID token"
	sys.exit()
...
try:
	...
	print "Test 5 PASS: Succesfully viewed products"
...
try:
	...
	print "Test 6 FAILED: Could not view products with category - fruit"
	print err.args
	sys.exit()
else:
	print "Test 6 PASS: Succesfully viewed products with category - fruit"